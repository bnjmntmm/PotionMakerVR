...
func _on_grab_collision_area_entered(area: Area3D) -> void:
	if area.name == "NewPotionArea":
		...
		var id = App.pickable_ingredients.find(pickable_ingredientName)
		##if id != -1 the correct was found-> set to currentObjectToSpawn
		if id != -1:
			currentObjectToSpawn = pickable_ingredientName
